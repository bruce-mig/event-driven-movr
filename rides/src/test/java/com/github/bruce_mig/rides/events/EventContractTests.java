...
import static com.github.bruce_mig.rides.util.TestHelpers.createRideEnded;
...
class EventContractTests {
    ...
    @Test
    public void rideEnded_shouldProduceTheExpectedJson() throws JsonProcessingException {
        RideEnded event = createRideEnded();
        ...
        assertEquals(7, eventMap.size());
        assertEquals(event.getRideId().toString(), eventMap.get("ride_id"));
        assertEquals(event.getUserEmail(), eventMap.get("user_email"));
        assertEquals(event.getVehicleId().toString(), eventMap.get("vehicle_id"));
        assertEquals(event.getBattery(), eventMap.get("battery"));
        assertEquals(event.getLatitude(), eventMap.get("latitude"));
        assertEquals(event.getLongitude(), eventMap.get("longitude"));
        assertEquals(event.getEndTime(), LocalDateTime.parse((String) eventMap.get("end_time")));
    }
}